...
# ============================================
# 数据访问方法（供 AchievementManager 等使用）
# ============================================

## 获取指定键的保存数据
func get_save_data(key: String) -> Variant:
    return current_save_data.get(key, {})

## 设置保存数据
func set_save_data(key: String, data: Variant) -> void:
    current_save_data[key] = data
    save_data_changed.emit()
